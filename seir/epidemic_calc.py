...
def getSolution(dfdt,pop,t0,days,group):
    if t0==0:
        ...
        T,S,E,I,R,Mild,Severe,Severe_H,Fatal,R_Mild,R_Severe,R_Fatal,intervention=list(np.arange(t0)+1),[0]*t0,[0]*t0,[0]*t0,[0]*t0,[0]*t0,[0]*t0,[0]*t0,[0]*t0,[0]*t0,[0]*t0,[0]*t0,[0]*t0
    ...
    D_death,P_SEVERE,D_hospital_lag,D_recovery_severe, \
    D_recovery_mild,CFR,rates,param_list,nodal_param_change,S0,delI,delS,delE,delR,rate_frac= itemgetter("D_incubation","D_infectious","I0","R0","E0","Mild0","Severe0","Severe_H0", \
                                                                                                         "Fatal0","R_Mild0","R_Severe0","R_Fatal0","D_death","P_SEVERE","D_hospital_lag", \
                                                                                                         "D_recovery_severe","D_recovery_mild","CFR","rates","param","nodal_param_change","S0","delI","delS","delE","delR","rate_frac")(group)
    try:
    ...
    return np.array(S[:days]),np.array(E[:days]),np.array(I[:days]),np.array(R[:days]),np.array(Mild[:days]),np.array(Severe[:days]),np.array(Severe_H[:days]),np.array(Fatal[:days]),np.array(R_Mild[:days]),np.array(R_Severe[:days]),np.array(R_Fatal[:days]),param_list
...
def epidemic_calculator(Config,days):
    S,E,I,R,Mild,Severe,Severe_H,Fatal,R_Mild,R_Severe,R_Fatal=np.array([0]*days),np.array([0]*days),np.array([0]*days),np.array([0]*days),np.array([0]*days),np.array([0]*days),np.array([0]*days),np.array([0]*days),np.array([0]*days),np.array([0]*days),np.array([0]*days)
    t0 = Config.t0
    for age_group in Config.age_group_dict:
        group = Config.age_group_dict[age_group]
        pop   = Config.pop*group['pop_frac']
        S0,E0,I0,R0,Mild0,Severe0,Severe_H0,Fatal0,R_Mild0,R_Severe0,R_Fatal0,intervention  =   getSolution(dfdt,pop,t0,days,group)
...
class GlobalConfig:
    ...
    def instantiate_with_local_config(self, local_config={"age_groups":{"0-20": {}, "21-40":{}, "41-60":{}, "61+":{}}}):
        local_config_params = {}
...
def network_epidemic_calc(city,days=200):
    ...
    if city =="India":
        for local_config in node_json_list:
            ...
            E0, Fatal0, I0, Mild0, R0, R_Fatal0, R_Mild0, R_Severe0, S0, Severe0, Severe_H0, intervention,node_config = get_SEIR(days,local_config)
        ...
        E, Fatal, I, Mild, R, R_Fatal, R_Mild, R_Severe, S, Severe, Severe_H, intervention,node_config = get_SEIR(days,local_config)
        t0=node_config.t0
